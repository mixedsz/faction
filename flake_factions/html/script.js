(function () {
    ...
    function renderMemberTerritoryTab(content) {
        ...
        const isBoss = content.member_rank === 'boss';
        ...
        territories.forEach(function(terr) {
            ...
            const manageBtn = (terr.type === 'stash' && isBoss)
                ? `<button class="btn-action-small btn-manage-stash" data-terr-id="${terr.id}" style="padding:6px 12px;font-size:0.78rem;background:#1d4ed8;border:1px solid #2563eb;border-radius:6px;color:#fff;cursor:pointer;margin-top:6px;">Manage Stash Access</button>`
                : '';
            html += `
        ...
        // Wire up "Manage Stash Access" buttons
        tabContent.querySelectorAll('.btn-manage-stash').forEach(function(btn) {
            btn.addEventListener('click', function() {
                const tid = parseInt(this.getAttribute('data-terr-id'), 10);
                if (tid) post('requestStashAccess', { territoryId: tid });
            });
        });
    }
    ...
    function renderMemberCooldownsTab(content) {
        ...
        if (cooldowns.length === 0) {
            ...
            cooldowns.forEach(cooldown => {
                ...
                html += `
                    <div class="cooldown-card" id="${cooldownId}" style="background: #27272a; border: 1px solid #3f3f46; border-radius: 10px; padding: 1rem; margin-bottom: 0.75rem;" data-secs-remaining="${secsRemaining}">
                        <div style="display: flex; align-items: center; gap: 1rem;">
                            <div class="faction-icon-small" style="background: #f59e0b; width: 40px; height: 40px; border-radius: 50%; display: flex; align-items: center; justify-content: center; color: #fff; font-weight: 700; font-size: 14px; flex-shrink: 0;">
        ...
        // Start live countdown timers — seed from seconds_remaining so there are
        // no timezone issues with parsing MySQL datetime strings in the browser.
        cooldowns.forEach(cooldown => {
            ...
            // Use the server-supplied seconds_remaining as the initial counter.
            let secsLeft = parseInt(cooldownCard.getAttribute('data-secs-remaining') || '0', 10);
            if (isNaN(secsLeft) || secsLeft <= 0) {
            ...
            const interval = setInterval(() => {
                ...
                if (secsLeft <= 0) {
                    ...
                    timerElement.textContent = formatTimeRemaining(secsLeft);
    ...
    window.addEventListener('message', function (event) {
        ...
        // ============================================================
        // STASH ACCESS MANAGEMENT MODAL
        // ============================================================
        if (data.action === 'stashAccessData') {
            const payload   = data.content || {};
            const members   = payload.members  || [];
            const whitelist = payload.whitelist || [];
            const tid       = payload.territory_id;

            // Build a set for quick lookup
            const whiteSet = new Set(whitelist);

            let html = `
                <div id="stash-access-modal" style="position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.7);z-index:9999;display:flex;align-items:center;justify-content:center;">
                    <div style="background:#18181b;border:1px solid #3f3f46;border-radius:12px;padding:24px;min-width:340px;max-width:500px;width:90%;">
                        <h3 style="color:#fff;margin:0 0 6px 0;font-size:1.1rem;">Manage Stash Access</h3>
                        <p style="color:#a1a1aa;font-size:0.8rem;margin:0 0 16px 0;">
                            Check members who are allowed to open this stash.<br>
                            Leave all unchecked to allow all faction members.
                        </p>
                        <div style="max-height:280px;overflow-y:auto;display:flex;flex-direction:column;gap:8px;margin-bottom:16px;">
            `;

            if (members.length === 0) {
                html += '<div style="color:#71717a;font-size:0.85rem;">No members found.</div>';
            } else {
                members.forEach(function(m) {
                    const checked = whiteSet.has(m.identifier) ? 'checked' : '';
                    const name    = escapeHtml(m.player_name || m.identifier);
                    const rankLbl = m.rank || 'member';
                    html += `
                        <label style="display:flex;align-items:center;gap:10px;cursor:pointer;padding:8px 10px;border-radius:6px;background:#27272a;border:1px solid #3f3f46;">
                            <input type="checkbox" data-ident="${escapeHtml(m.identifier)}" ${checked} style="width:16px;height:16px;accent-color:#2563eb;cursor:pointer;">
                            <div>
                                <div style="color:#fff;font-size:0.875rem;font-weight:500;">${name}</div>
                                <div style="color:#71717a;font-size:0.75rem;">${escapeHtml(rankLbl)}</div>
                            </div>
                        </label>
                    `;
                });
            ...
            html += `
                        ...
                        <div style="display:flex;gap:8px;justify-content:flex-end;">
                            <button id="stash-access-cancel" style="padding:8px 16px;background:#27272a;border:1px solid #3f3f46;border-radius:6px;color:#a1a1aa;cursor:pointer;font-size:0.875rem;">Cancel</button>
                            <button id="stash-access-save" style="padding:8px 16px;background:#2563eb;border:none;border-radius:6px;color:#fff;cursor:pointer;font-size:0.875rem;font-weight:600;">Save Access List</button>
                        </div>
                    </div>
                </div>
            `;

            // Inject modal into document body (not tabContent, so it overlays everything)
            const existingModal = document.getElementById('stash-access-modal');
            if (existingModal) existingModal.remove();
            const wrapper = document.createElement('div');
            wrapper.innerHTML = html;
            document.body.appendChild(wrapper.firstElementChild);

            document.getElementById('stash-access-cancel').addEventListener('click', function() {
                const modal = document.getElementById('stash-access-modal');
                if (modal) modal.remove();
            });

            document.getElementById('stash-access-save').addEventListener('click', function() {
                const checkboxes = document.querySelectorAll('#stash-access-modal input[type="checkbox"]:checked');
                const selected   = Array.from(checkboxes).map(function(cb) { return cb.getAttribute('data-ident'); });
                post('saveStashAccess', { territoryId: tid, whitelist: selected });
                const modal = document.getElementById('stash-access-modal');
                if (modal) modal.remove();
            });
        }
