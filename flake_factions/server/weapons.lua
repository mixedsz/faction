...
local function RunScheduledGunDrop()
    ...
    -- Fetch eligible factions with current reputation straight from faction_factions
    ...
    if not eligibleFactions or #eligibleFactions == 0 then
        print('[flake_factions] No gun-drop-eligible factions found.')
        return
    end

    print(string.format('[flake_factions] Checking %d eligible faction(s) for gun drop (min rep: %d)...', #eligibleFactions, Config.GunDrops.minReputation))
    ...
    for _, faction in ipairs(eligibleFactions) do
        local rep = faction.reputation or 0
        if rep < Config.GunDrops.minReputation then
            -- Log actual DB value so admins can diagnose
            print(string.format('[flake_factions] Skipping %s — faction_factions.reputation = %d (need %d). '
                .. 'Note: reputation_contribution in faction_members tracks personal kills only; '
                .. 'faction reputation reflects net combat rep and can drop from member deaths.',
                faction.label, rep, Config.GunDrops.minReputation))
        else
            local weapons = MySQL.query.await([[
                SELECT weapon_name, serial_number, weapon_hash
                FROM faction_weapons
...
local REP_KILL_COOLDOWN  = 3    -- seconds — only blocks duplicate network events, not legit back-to-back kills
local REP_DEATH_COOLDOWN = 10
local UNARMED_HASH = joaat('weapon_unarmed')

-- Fired by the VICTIM'S client: "killerServerId killed me with weaponHash"
-- Victim-side detection is used because the attacker ped is guaranteed alive
-- (and thus findable), whereas the dead victim ped can vanish before the
-- attacker's GetActivePlayers() loop runs.
RegisterNetEvent('faction:victimReportsKiller', function(killerServerId, weaponHash)
    local source = source  -- victim's server ID

    -- Server-side unarmed guard
    local wHash = tonumber(weaponHash) or 0
    if wHash == UNARMED_HASH or wHash == 0 then return end

    local killer = ESX.GetPlayerFromId(tonumber(killerServerId))
    if not killer then return end

    local killerRow = GetPlayerFactionData(killer.identifier)
    if not killerRow then return end  -- killer not in a faction

    -- Cooldown: rate-limit rep per killer so bursts don't spam
    local now = os.time()
    if (now - (killRepCooldowns[killer.identifier] or 0)) < REP_KILL_COOLDOWN then return end
    killRepCooldowns[killer.identifier] = now

    -- Victim faction lookup (for friendly-fire check only)
    local victim    = ESX.GetPlayerFromId(source)
    local victimRow = victim and GetPlayerFactionData(victim.identifier) or nil

    -- Friendly-fire: no rep for killing own faction member
    if victimRow and victimRow.faction_id == killerRow.faction_id then return end

    -- Rep amount: enemy faction kill vs civilian kill
    local memberGain
    local isEnemyKill = (victimRow ~= nil)

    if isEnemyKill then
        memberGain = tonumber(Config.Reputation.killEnemy) or 5
    else
        memberGain = math.random(
            tonumber(Config.Reputation.killCivilianMin) or 1,
            tonumber(Config.Reputation.killCivilianMax) or 3
        )
    end

    local rate        = tonumber(Config.Reputation.factionContribRate) or 1.0
    local factionGain = math.max(1, math.floor(memberGain * rate))

    MySQL.update([[
        UPDATE faction_members
        SET reputation_contribution = reputation_contribution + ?
        WHERE identifier = ? AND faction_id = ?
    ]], { memberGain, killer.identifier, killerRow.faction_id })

    MySQL.update('UPDATE faction_factions SET reputation = reputation + ? WHERE id = ?',
        { factionGain, killerRow.faction_id })

    local killType = isEnemyKill and 'enemy faction kill' or 'street kill'
    lib.notify(killer.source, {
        type = 'success', title = 'Reputation',
        description = string.format('+%d personal  +%d faction  (%s)',
            memberGain, factionGain, killType),
        duration = 5000
    })

    SendFactionDataToPlayer(killer.source)

    if Config.Webhooks.enabled and Config.Webhooks.weaponLogging ~= '' then
        PerformHttpRequest(Config.Webhooks.weaponLogging, function() end, 'POST',
            json.encode({ content = string.format(
                '**Kill** | Killer: %s | Faction: %s | Victim: %s | %s | Personal +%d | Faction +%d',
                killer.getName(), killerRow.faction_label,
                victim and victim.getName() or ('SrvID-'..tostring(source)),
                killType, memberGain, factionGain) }),
            { ['Content-Type'] = 'application/json' })
    end
end)

-- Keep old event name as a no-op shim so any cached client versions don't error
RegisterNetEvent('faction:playerKilledPlayer', function() end)

-- Called when the local player is killed by a weapon (punching excluded client-side)
RegisterNetEvent('faction:playerDied', function()
    ...
    local memberLoss  = math.abs(Config.Reputation.memberDeathLoss or 5)
    local factionLoss = math.abs(Config.Reputation.loseMember or 10)

    -- Member's personal reputation goes down (floored at 0)
    MySQL.update([[
        UPDATE faction_members
        SET reputation_contribution = GREATEST(0, reputation_contribution - ?)
        WHERE identifier = ? AND faction_id = ?
    ]], { memberLoss, xPlayer.identifier, row.faction_id })

    -- Faction reputation goes down (floored at 0)
    MySQL.update('UPDATE faction_factions SET reputation = GREATEST(0, reputation - ?) WHERE id = ?', { factionLoss, row.faction_id })

    lib.notify(source, {
        type = 'error', title = 'Reputation',
        description = string.format('-%d personal rep / -%d faction rep (killed in combat)', memberLoss, factionLoss)
    })

    if Config.Webhooks.enabled and Config.Webhooks.weaponLogging ~= '' then
        PerformHttpRequest(Config.Webhooks.weaponLogging, function() end, 'POST',
            json.encode({ content = string.format(
                '**Member Death** | Member: %s | Faction: %s | Member -%d Rep | Faction -%d Rep',
                xPlayer.getName(), row.faction_label, memberLoss, factionLoss) }),
            { ['Content-Type'] = 'application/json' })
    end
end)
