...
-- Reputation System
Config.Reputation = {
    -- Per enemy faction kill (killing a member of a DIFFERENT faction):
    --   Member's reputation_contribution increases by killEnemy.
    --   Faction reputation increases by killEnemy * factionContribRate.
    killEnemy = 5,

    -- Per civilian kill (killing a player NOT in any faction):
    --   Grants a random amount between killCivilianMin and killCivilianMax.
    killCivilianMin = 1,
    killCivilianMax = 3,

    -- Fraction of a member's kill rep that flows into faction_factions.reputation.
    -- 1.0 = 100% (full kill rep goes to faction as well as member).
    -- 0.6 = 60% (member gets +5, faction gets +3 per kill).
    factionContribRate = 1.0,

    -- On weapon death:
    --   Member's reputation_contribution decreases by memberDeathLoss (floored at 0).
    --   Faction reputation decreases by loseMember (floored at 0).
    memberDeathLoss = 5,  -- personal rep member loses when killed
    loseMember = 10,      -- faction rep lost when a member is killed

    winWar = 50,          -- faction rep gained for winning a war
    loseWar = 25,         -- faction rep lost for losing a war
    territoryClaim = 20,  -- faction rep gained for claiming territory
    territoryLost = 15    -- faction rep lost for losing territory
}
