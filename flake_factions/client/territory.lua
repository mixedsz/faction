...
-- ============================================================
-- SPHERE DRAW THREAD
-- DrawMarker type 26 = mp_alertsphere (the actual translucent 3D sphere).
-- Type 28 is a flat circle — wrong type.
-- For type 26, scaleX/Y/Z = DIAMETER (radius * 2) to match the zone radius.
    ...
    while true do
        if #turfSphereDrawList > 0 then
            for _, s in ipairs(turfSphereDrawList) do
                local diam = s.radius * 2.0   -- type 26 scale = diameter
                DrawMarker(
                    26,                    -- mp_alertsphere — the 3D translucent sphere
                    s.x, s.y, s.z,         -- centre position
                    0.0, 0.0, 0.0,         -- direction
                    0.0, 0.0, 0.0,         -- rotation
                    diam, diam, diam,       -- diameter, so visual radius == zone radius
                    s.r, s.g, s.b, 55,     -- colour + alpha (55/255 = subtle but visible)
