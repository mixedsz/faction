...
-- ============================================================
-- STASH ACCESS MANAGEMENT (Boss only)
-- ============================================================

-- NUI requests the current whitelist for a stash territory
RegisterNUICallback('requestStashAccess', function(data, cb)
    cb('ok')
    TriggerServerEvent('faction:getStashAccess', data.territoryId)
end)

-- Server replies with whitelist + member list
RegisterNetEvent('faction:receiveStashAccess', function(payload)
    SendNUIMessage({
        action  = 'stashAccessData',
        content = payload  -- { territory_id, whitelist = [...], members = [...] }
    })
end)

-- NUI submits the updated whitelist
RegisterNUICallback('saveStashAccess', function(data, cb)
    cb('ok')
    -- data.territoryId (number), data.whitelist (array of identifier strings)
    TriggerServerEvent('faction:setStashAccess', data.territoryId, data.whitelist or {})
end)
